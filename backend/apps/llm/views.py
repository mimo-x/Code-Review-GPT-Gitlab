...
import tempfile
import subprocess
from .models import LLMConfig, GitLabConfig, NotificationConfig, NotificationChannel, WebhookEventRule, OpenCodeCliConfig
...
class ConfigViewSet(viewsets.GenericViewSet):
    ...
    def summary(self, request):
        ...
        active_opencode_cli_config = OpenCodeCliConfig.objects.filter(is_active=True).first()
        ...
        data = {
            ...
            'opencode_cli': OpenCodeCliConfigSerializer(active_opencode_cli_config).data if active_opencode_cli_config else None,
    ...
    def batch_update(self, request):
        ...
        try:
            ...
            # 更新 OpenCode CLI 配置
            if 'opencode_cli' in data:
                opencode_cli_data = data['opencode_cli']
                active_opencode_cli = OpenCodeCliConfig.objects.filter(is_active=True).first()

                if active_opencode_cli:
                    opencode_cli_serializer = OpenCodeCliConfigSerializer(active_opencode_cli, data=opencode_cli_data, partial=True)
                    if opencode_cli_serializer.is_valid():
                        opencode_cli_serializer.save()
                    else:
                        errors['opencode_cli'] = opencode_cli_serializer.errors
                else:
                    opencode_cli_serializer = OpenCodeCliConfigSerializer(data=opencode_cli_data)
                    if opencode_cli_serializer.is_valid():
                        opencode_cli_serializer.save()
                    else:
                        errors['opencode_cli'] = opencode_cli_serializer.errors
    ...
    @action(detail=False, methods=['post'], url_path='test-opencode-cli')
    def test_opencode_cli(self, request):
        """测试 OpenCode CLI 配置是否可用"""
        try:
            from apps.review.opencode_cli_service import OpenCodeCliService

            cli_payload = request.data.get('opencode_cli', {})
            service = OpenCodeCliService()
            service.auth_content = cli_payload.get('auth_content') or service.auth_content
            service.config_content = cli_payload.get('config_content') or service.config_content
            service.env_content = cli_payload.get('env_content') or service.env_content
            service.timeout = cli_payload.get('timeout', service.timeout)
            ...
            success, version, version_error = service.get_cli_version()
            if not success:
                logger.warning(f"无法获取 OpenCode CLI 版本: {version_error}")
                version = 'unknown'
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with open(file_path, 'w', encoding='utf-8') as handler:
                    handler.write('demo data')
                subprocess.run(['git', 'add', '.'], cwd=temp_dir, capture_output=True)
                subprocess.run(['git', 'commit', '-m', 'init'], cwd=temp_dir, capture_output=True)

                ok, _, err = service.review_code(temp_dir, commit_range=None, custom_prompt="Ping")
                if not ok:
                    return Response({'status': 'error', 'message': err}, status=status.HTTP_400_BAD_REQUEST)

            return Response({
                'status': 'success',
                'message': 'OpenCode CLI 配置正确，执行成功',
                'version': version,
                'details': {
                    'cli_path': service.cli_path,
                    'auth_configured': bool(service.auth_content),
                    'config_configured': bool(service.config_content),
                    'env_configured': bool(service.env_content),
                }
            })
        except subprocess.TimeoutExpired:
            return Response({
                'status': 'error',
                'message': 'OpenCode CLI 调用超时，请检查 CLI 配置或网络'
            }, status=status.HTTP_400_BAD_REQUEST)
        except Exception as exc:
            logger.error(f"测试 OpenCode CLI 配置失败: {exc}", exc_info=True)
            return Response({
                'status': 'error',
